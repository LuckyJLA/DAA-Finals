...
for i in range(2):
    if i == 0:
        rider = knapsack(group1, 10000)
    else:
        rider = knapsack(group2, 10000)

    adj_mat, paths_addr = calculate_shortest_path(rider, graph)
    ...
    for j in range(len(shortest_path) - 1):
        ...
        print("Edge:", current_node, "->", next_node, "Distance:", adj_mat[current_node][next_node])
...
marker = folium.Marker(location=(14.60316,120.99233), popup=f"Coordinates: {14.60316}, {120.99233}")
# Add tooltip with the name of the coordinate
tooltip = folium.Tooltip(text="HUB")
