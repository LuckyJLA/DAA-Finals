...
csv_reader = pd.read_csv(r'Final Program\\Packages.csv')
...
print("\n\nParcel Group 1")
print("Parcel ID\tWeight(kg)\tPrice(Php)")
for i in group1:
    
    print(f"{i[2]:<9}\t{i[3]/1000:<9}\t{i[4]/1000:<9.2f}")
...
for i in range(2):
    ...
    print("Parcel ID\tWeight(kg)\tPrice(Php)")
    for x in rider:
        print(f"{x[2]:<9}\t{x[3]/1000:<9}\t{x[4]/1000:<9.2f}")
        total_weight += x[3]/1000
        total_price += x[4]/1000

    print(f"Total Weight: {total_weight:.5f} kg")
    print(f"Total Price: {total_price:.2f} Php")
    ...
    for j in range(len(shortest_path) - 1):
        ...
        print(f"Edge: {rider[current_node][2]} -> {rider[next_node][2]} Distance: {adj_mat[current_node][next_node]/1000:.5f} km")
    ...
    print(f"\nThe Shortest Path Distance: {sum:.5f} km")
...
view.save(r'Final Program\\mapping.html')
#opens file
wb.open(r'Final Program\\mapping.html')
...
Alamag, Jose Luis
Marcellana, Nicole
Reyes, Mark Angelo III
Suarez, Edrin Ivan
Tolentino, Winzelle Anne
Villegas, Daniel
'''